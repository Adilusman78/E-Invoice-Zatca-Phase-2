report 70501 "Credit Arabic Invoice DSSS"
{
    DefaultLayout = RDLC;
    ApplicationArea = all;
    UsageCategory = ReportsAndAnalysis;
    Caption = 'Arbic Invoice';
    RDLCLayout = './ReportsLayout/Arabic Invoice.rdl';

    dataset
    {
        dataitem(SalesInvoicHeader; "Sales Cr.Memo Header")
        {
            ...
            column(Order_Date; Format("Shipment Date")) //AHamza Change the "Order Date" to "Shipment Date"
            {
            }
    ...
    local procedure InitArguments(DocNo: text): text
    var
    ...
    begin
        ...
        // TLV - 3
        TLBinaryVal := DecToBinUTF8(3) + DecToBinUTF8(StrLen(format(CurrentDateTime())));
        QrCodeParameter := QrCodeParameter + Base64ConvertDSSS.BinaryToText(TLBinaryVal);
        QrCodeParameter := QrCodeParameter + Format(CurrentDateTime);
        // TLV - 3 Ends
        ...
        if HttpClient.get('https://barcode.tec-it.com/barcode.ashx?data=' + QrCodeParameter + '&code=QRCode', HttpResponseMessage) then begin
